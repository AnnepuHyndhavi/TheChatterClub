...
<html>
<head>
    <title>Social Networking Site</title>       <!--title to be shown-->
    <link rel="stylesheet" type="text/css" href="f_end/css/main.css">  <!--basic css is used in main.css-->
    <style>
    .post{
        margin-right: 50px;
        float: right;
...
<body>
    <div class="container">
        ...
        if($flag == 0) { // One's Own Profile
        ...
        } else { // From Another Profile ---> Retrieve the User data and their friendship status
            $profilesql = "SELECT users.user_id, users.user_gender, users.user_hometown, users.user_status, users.user_birthdate,
            ...
            if(isset($row['friendship_status'])){ // Either or requested as a friend
                if($row['friendship_status'] == 1){ // friends
                    $postsql = "SELECT posts.post_caption, posts.post_time, users.user_firstname, users.user_lastname,
                                        posts.post_public, users.user_id, users.user_gender, users.user_nickname,
                                        users.user_birthdate, users.user_hometown, users.user_status, users.user_about,
                                        posts.post_id
                ...
                else if($row['friendship_status'] == 0){ // Requested to be a Friend
                    $postsql = "SELECT posts.post_caption, posts.post_time, users.user_firstname, users.user_lastname,
                                        posts.post_public, users.user_id, users.user_gender, users.user_nickname,
                                        users.user_birthdate, users.user_hometown, users.user_status, users.user_about,
                                        posts.post_id
        ...
        if($postquery){
            ...
            if(mysqli_num_rows($postquery) == 0){ // No Posts
                if($flag == 0){ // Message is shown if it is your own profile.
                    echo '<div class="post">';
                    echo 'You didn\'t make/have any posts yet';
                    echo '</div>';
                } else { // Message shown if it's another profile than yours.
                    echo '<div class="post">';
                    echo 'There are no public posts to show right now.';
                    echo '</div>';
                }
                include 'imp/profile1.php';
            } else {
                ...
                <br>
                <div class="profile">
                    <center class="changeprofile">Add Phone Number here</center>
                    <br>
...
function validateNumber(){
    ...
    if(number == ""){
        required[0].innerHTML = "You must type Your Number here.";
        return false;
    } else if(isNaN(number)){
        required[0].innerHTML = "Phone Number must contain the digits only."
        return false;
    }
    return true;
}
